  ...
  <footer class="bg-gray-800 text-white py-12 mt-16">
    <div class="max-w-6xl mx-auto px-4">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul class="space-y-2">
            ...
            <li><a href="complaints.php" class="text-gray-400 hover:text-white transition-colors">Submit Complaint</a></li>
          </ul>
        </div>
        ...
        <!-- Newsletter (optional for future use) -->
        <div>
          <h3 class="text-lg font-bold mb-4">Stay Updated</h3>
          <p class="text-gray-400 mb-2">Subscribe to receive updates on our referral program.</p>
          <form class="flex">
            <input type="email" placeholder="Your email" class="px-4 py-2 w-full rounded-l focus:outline-none text-gray-900">
            <button type="submit" class="bg-indigo-600 px-4 py-2 rounded-r hover:bg-indigo-700 transition-colors">
              Subscribe
            </button>
          </form>
        </div>
      ...
      <div class="border-t border-gray-700 mt-8 pt-8 flex flex-col md:flex-row justify-between items-center">
        <p class="text-gray-400 text-sm mb-4 md:mb-0">ISP Ledger Referral Program - Building stronger networks together</p>
  ...
  <script>
    // Tab switching logic for Weekly/Monthly views
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (weeklyMainTab && monthlyMainTab && weeklySection && monthlySection) {
        weeklyMainTab.addEventListener('click', function() {
          weeklyMainTab.classList.add('bg-indigo-600', 'text-white');
          weeklyMainTab.classList.remove('bg-gray-200', 'text-gray-700');
        ...
        monthlyMainTab.addEventListener('click', function() {
          monthlyMainTab.classList.add('bg-indigo-600', 'text-white');
          monthlyMainTab.classList.remove('bg-gray-200', 'text-gray-700');
      ...
      if (weeklyTabs.length > 0) {
        weeklyTabs.forEach(tab => {
          tab.addEventListener('click', function() {
            ...
            document.querySelectorAll('.weekly-content').forEach(content => {
        ...
        monthlyTabs.forEach(tab => {
          tab.addEventListener('click', function() {
            ...
            monthlyTabs.forEach(t => {
              t.classList.remove('bg-green-600', 'text-white', 'shadow-green-200');
              t.classList.add('bg-white', 'text-gray-700');
            });
            
            // Highlight clicked tab
            this.classList.remove('bg-white', 'text-gray-700');
            this.classList.add('bg-green-600', 'text-white', 'shadow-green-200');
            ...
            document.getElementById('month' + monthNum).classList.remove('hidden');
          });
        });
      }
    });
